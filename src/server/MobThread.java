...
public class MobThread extends Control {
	...
	private int intervalo = Constantes.DEFAULT_INTERVALO;
	/** Indice del vector de rutinas */
	private int index = 0;
	/** Mobprogs */
	private HashMap<Constantes.MobProgs, List<String>> progs;
	 ...
	 */
	public MobThread(Player p, Habitacio h) {
		super("MobThread", h, p);
		progs = new HashMap<Constantes.MobProgs, List<String>>();
	}
	...
	public void send(String mensaje) {
		...
		if (mensaje.indexOf("te dice") > 0) {
			run_progs(Constantes.MobProgs.SPEECH, ptarget);
		} else if (mensaje.indexOf("te da") > 0) {
			run_progs(Constantes.MobProgs.GIVE, ptarget);
		}
	}
	 ...
	 */
	private Object run_progs(Constantes.MobProgs tipo, Player target) {
		Object result = null;
	...
	public void run() {
		...
		try {
			while (true) {
				...
				case FIGHT:
					run_progs(Constantes.MobProgs.FIGHT, player.getVictim());
					break;
				default: // NORMAL
					run_progs(Constantes.MobProgs.RAND, null);
					if (script != null) {
	...
	public int getIntervalo() {
		return intervalo / Constantes.SEGUNDOS;
	}
	...
	public void setIntervalo(int intervalo) {
		this.intervalo = intervalo * Constantes.SEGUNDOS;
	}
	 ...
	 */
	public void addProg(Constantes.MobProgs tipo, String prog) {
